...
p = annotate!(p, N/2, -2, text(L"(i, j) element of Higham$^2$", 14, :center))

center_x = N * 0.5
center_y = N * 0.5

#
p = annotate!(p, center_x, center_y, "Maximum change occurs by\nperturbing the (1,1) entry", font(12, :white))
margin = 1.5
p = plot!(p, [center_x-1, margin], [center_y-1.5, margin], legend=false, color="black")
p = plot!(p, [margin, margin+0.3], [margin, margin], legend=false, color="black")
p = plot!(p, [margin, margin], [margin, margin+0.3], legend=false, color="black")

#
margin_x = 1.7
margin_y = 1.2
p = annotate!(p, center_x, margin_y, "Smallest last pivot", font(12))
p = plot!(p, [center_x-5, margin_x], [margin_y, margin_y], legend=false, color="black")
p = plot!(p, [margin_x, margin_x+0.25], [margin_y, margin_y+0.25], legend=false, color="black")
p = plot!(p, [margin_x, margin_x+0.25], [margin_y, margin_y-0.25], legend=false, color="black")

savefig("/mnt/data/code/Plots_final/heatmap_products/heatmap_higham_" * string(N) * ".png")
